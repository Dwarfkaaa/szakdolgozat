...
@section('content')
    ...
    @media only screen and (min-width:320px) and (max-width: 650px) {
        ...
        #mceu_23-inp, #mceu_24, #mceu_25-inp, #mceu_26-inp{
...
<div id="chatter" class="discussion" style="display:block;  background-repeat: repeat;  background-attachment: fixed;  background-size: cover;
    background-position: center center; height: auto; min-height: 100vh;
    background-image: url({{asset('images/bg.jpeg')}});">

	<div id="chatter_header" style="background-color: #262626;" >
		<div class="container" >
			<a class="back_btn"  href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-back"></i></a>
			<h1 style="">{{ $discussion->title }}</h1><span class="chatter_head_details"> @lang('chatter::messages.discussion.head_details')<a class="chatter_cat" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
	                ...
	                <ul class="discussions no-bg" style="display:block;">
	                	@foreach($posts as $post)
	                		<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
		                		<span class="chatter_posts" style="background: initial; border-top: 0; !important;">
		                			@if(!Auth::guest() && (Auth::user()->id == $post->user->id))
		                				<div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete">
		                					...
		                					<button class="btn btn-sm btn-danger pull-center delete_response">@lang('chatter::messages.response.yes_confirm')</button>
		                					<button class="btn btn-sm btn-default pull-center">@lang('chatter::messages.response.no_confirm')</button>
		                				</div>
			                			<div class="chatter_post_actions">
			                				<p class="chatter_delete_btn" style="background-color: black ">
			                					<i class="chatter-delete"></i> @lang('chatter::messages.words.delete')
			                				</p>
			                				<p class="chatter_edit_btn" style="background-color: black ">
					        		...
					        		<div class="chatter_middle">
					        			<span class="chatter_middle_details"><a style="color: white" href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
					        			<div class="chatter_body" style="color: white">

					        				@if($post->markdown)
					        					<pre class="chatter_body_md" style="color: white">{{ $post->body }}</pre>
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			...
		        			@else
		        				<span> <img  src="images/avatars/{{Auth::user()->avatar}}">
		        				</span>
		        			@endif
		        		</div>

			            <div id="new_discussion" style="z-index: 0; border-radius: 10px;">
						...
						<div id="discussion_response_email">
							<button id="submit_response" class="btn btn-success pull-center"><i class="chatter-new"></i> @lang('chatter::messages.response.submit')</button>
        ...
        <div id="new_discussion_in_discussion_view">
            ...
            <form id="chatter_form_editor_in_discussion_view" action="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
                ...
                <div id="new_discussion_footer">
                    ...
                    <button id="submit_discussion" class="btn btn-success pull-center"><i class="chatter-new"></i> Create {{ Config::get('chatter.titles.discussion') }}</button>
                    <a href="/{{ Config::get('chatter.routes.home') }}" class="btn btn-default pull-center" id="cancel_discussion">Cancel</a>
...
<script>
	$('document').ready(function(){
		...
		$('.chatter_edit_btn').click(function(){
			...
			container.append('<div class="chatter_update_actions" style="background: initial;border: 0;padding-top: 3%;"><button class="btn btn-success pull-center update_chatter_edit"  data-id="' + id + '" data-markdown="' + markdown + '"><i class="chatter-check"></i> @lang('chatter::messages.response.update')</button><button href="/" class="btn btn-default pull-center cancel_chatter_edit" style="color: white" data-id="' + id + '"  data-markdown="' + markdown + '">@lang('chatter::messages.words.cancel')</button></div>');
